...
def main():
    ...
    ir, node_sites = re.subn(
        r"(  %PersistentMapNode\.new\d* = call ptr @janus_alloc)",
    ...
    for mode in range(20):
        ...
        callback_panic = 9 <= mode <= 15
        if normal.returncode != (134 if callback_panic else 0) or not match or (normal.stderr and not callback_panic):
            raise RuntimeError(f"mode {mode}: {normal.stdout}\n{normal.stderr}")
        if callback_panic and ("allocation failed:" not in normal.stderr or "Sanitizer" in normal.stderr or "runtime error:" in normal.stderr):
            raise RuntimeError(normal.stderr)
        expected_nodes = {0: 3, 1: 3, 2: 1}
        if mode in expected_nodes and int(match[2]) != expected_nodes[mode]:
            raise RuntimeError(f"mode {mode}: unexpected rebuilt node count {match[2]}")
        count = int(match[1])

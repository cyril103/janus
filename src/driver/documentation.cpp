...
StructuredDocumentation parse_documentation(std::string_view source) {
  ...
  while (std::getline(lines, line)) {
    ...
    if (stripped.starts_with("@deprecated")) {
      result.deprecated = true;
      result.replacement = trim(std::string_view{stripped}.substr(11));
      if (result.replacement.starts_with("use "))
        result.replacement = trim(
            std::string_view{result.replacement}.substr(4));
      if (result.replacement.starts_with("[[") &&
          result.replacement.ends_with("]]"))
        result.replacement = result.replacement.substr(
            2, result.replacement.size() - 4);
      continue;
    }
...
<div id="empty-state" class="empty-state"><strong>No API entries match this search.</strong><br>Try a module name, symbol or type.</div>
  ...
  for (const auto &[module, documentation] : module_documentation) {
    ...
    html << "</div>";
    if (!module_doc.examples.empty()) {
      html << "<section class=\"module-example doc-section\"><h3>Usage example";
      if (module_doc.examples.size() != 1)
        html << 's';
      html << "</h3>";
      for (const std::string &example : module_doc.examples)
        html << "<pre class=\"example\"><code>"
             << render_documentation(example, module, links,
                                     report.unresolved_links, strict_links)
             << "</code></pre>";
      html << "</section>";
    }
    ...
    for (const Symbol &symbol : symbols) {
      ...
      if (symbol.structured.deprecated) {
        html << "<span class=\"deprecated-badge\">Deprecated</span>";
        if (!symbol.structured.replacement.empty())
          html << "<p>Use <code>" << html_escape(symbol.structured.replacement)
               << "</code> instead.</p>";
      }

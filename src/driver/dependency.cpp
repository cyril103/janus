...
std::map<std::string, janus::driver::RegistryLock>
locked_registry_records(const std::filesystem::path &path) {
  std::ifstream input{path};
  std::map<std::string, janus::driver::RegistryLock> result;
  std::string name;
  janus::driver::RegistryLock record;
  const auto commit = [&]() {
    if (!name.empty() && !record.registry.empty() && !record.version.empty() &&
        !record.metadata_sha256.empty() && !record.archive_sha256.empty())
      result[name] = record;
  };
  std::string line;
  while (std::getline(input, line)) {
    ...
    if (line == "[[dependency]]") {
      ...
      name = quoted_value();
    else if (line.starts_with("registry = "))
      record.registry = quoted_value();
    else if (line.starts_with("version = "))
      record.version = quoted_value();
    else if (line.starts_with("metadata-sha256 = "))
      record.metadata_sha256 = quoted_value();
    else if (line.starts_with("archive-sha256 = "))
      record.archive_sha256 = quoted_value();
  }
  commit();
  return result;
}
...
#endif
    if (error)
      throw std::runtime_error{"cannot publish janus.lock: " + error.message()};
...
resolve_dependencies(const Manifest &manifest,
  ...
  const std::map<std::string, std::string> locked =
      options.locked ? locked_versions(lock_path)
                     : std::map<std::string, std::string>{};
  const std::map<std::string, RegistryLock> registry_locks =
      options.locked ? locked_registry_records(lock_path)
                     : std::map<std::string, RegistryLock>{};
  ...
  std::map<std::string, ResolvedSource> resolved_sources;
  std::set<std::string> visiting;

  const auto resolve = [&](const auto &self, const Manifest &owner,
                           const Dependency &dependency) -> void {
    std::filesystem::path root;
    std::optional<ResolvedRegistryPackage> remote;
    if (dependency.is_registry()) {
      const std::string location = dependency.registry.empty()
                                       ? registry_location()
                                       : dependency.registry;
      if (is_remote_registry(location)) {
        const auto found = registry_locks.find(dependency.name);
        if (options.locked && found == registry_locks.end())
          throw std::runtime_error{
              "janus.lock is missing the verified registry record for '" +
              dependency.name + "'"};
        remote = resolve_remote_package(
            dependency,
            found == registry_locks.end()
                ? std::optional<RegistryLock>{}
                : std::optional<RegistryLock>{found->second},
            options.offline, cache_root());
        root = remote->root;
      } else {
    ...
    const std::string source_identity =
        dependency.is_registry()
            ? (remote ? "registry+" + remote->lock.registry + "#" +
                            dependency.name + "@" + remote->lock.version
                      : "registry+" + std::filesystem::weakly_canonical(root)
                                          .generic_string())
            : (dependency.is_git()
    ...
    if (visiting.contains(dependency.name))
      throw std::runtime_error{"cyclic dependency involving '" +
                               dependency.name + "'"};
    const auto validate_version = [&](const ResolvedSource &source) {
      if (!matches_version(dependency.version_requirement, source.version))
        throw std::runtime_error{
            "dependency '" + dependency.name + "' resolved to " +
            source.declared_version + ", which does not satisfy '" +
            dependency.version_requirement + "' required by '" + owner.name +
            "'"};
    };
    if (const auto existing = resolved_sources.find(dependency.name);
        existing != resolved_sources.end()) {
      if (existing->second.identity != source_identity)
        throw std::runtime_error{"dependency source conflict for '" +
                                 dependency.name + "'"};
      validate_version(existing->second);
      return;
    }
    ...
    const ResolvedSource source{
        source_identity, parse_semantic_version(dependency_manifest.version),
        dependency_manifest.version};
    validate_version(source);
    resolved_sources.emplace(dependency.name, source);
    lock += "\n[[dependency]]\nname = \"" + dependency.name + "\"\n";
    if (dependency.is_registry()) {
      if (remote) {
        lock += "source = \"registry+" + remote->lock.registry + "\"\n";
        lock += "registry = \"" + remote->lock.registry + "\"\n";
        lock += "version = \"" + dependency_manifest.version + "\"\n";
        lock += "metadata-sha256 = \"" + remote->lock.metadata_sha256 + "\"\n";
        lock += "archive-sha256 = \"" + remote->lock.archive_sha256 + "\"\n";
      } else {

...
`Vector2`, `Rectangle`, `Color`, `Key`, `MouseButton`, `TextureFilter`,
`BlendMode`, `GamepadButton` et `GamepadAxis` dérivent explicitement `Copy`,
`Equality` et `Debug`. Leur copie ne possède aucun handle et n'exécute aucun
nettoyage.

Les six classes suivantes sont non copiables :
...
Un chargement invalide retourne un handle nul pour les six familles.
...
La révision ne retire aucun des 317 symboles des six modules graphiques.
...
fixture Janus déplace les six ressources, détruit certaines explicitement et
...
Les six modules passent de 1 366 à 1 368 lignes hors documentation et fixtures,
soit **+0,15 %**, sous le budget de croissance de 5 %. Les garde-fous de portée
ajoutent uniquement des tests et affectations en temps constant, sans
allocation sur les chemins de dessin.
